...
import { ArrowLeft, ShoppingCart, MessageSquare, AlertCircle, Package, Trash2, Flag, Heart, Pencil } from 'lucide-react';
            ...
            <div className="flex gap-2 overflow-x-auto">
              {product.imageUrls.map((url, index) => (
                <button
                  ...
                  className={`w-16 h-16 rounded-lg border overflow-hidden cursor-pointer transition-colors shrink-0 ${
                    selectedImage === index ? 'border-slate-900 ring-1 ring-slate-900' : 'border-slate-200 hover:border-slate-400'
                  }`}
                >
                  <img src={url} className="w-full h-full object-cover" alt={`${product.title} ${index + 1}`} />
                </button>
              ))}
            </div>
          )}
            ...
            {user?.id === product.sellerId ? (
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="py-2.5 bg-white text-slate-700 border border-slate-200 rounded-lg font-medium text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-1.5"
                >
